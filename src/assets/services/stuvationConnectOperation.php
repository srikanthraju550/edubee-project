...
$response.="{";
$creatorEmailFinder = mysqli_fetch_row(mysqli_query($conn,"select b.emailaddress from stuvationdetails a, logindetails b where a.projectid=$projectid and a.userid = b.userid"));
$senderEmailFinder = mysqli_fetch_row(mysqli_query($conn,"select b.emailaddress from logindetails b where b.userid=$registereduserid"));
$emailContent = '<head>
  ...
  $subject="Stuvation Contact Email";
  $headers = "MIME-Version: 1.0" . "\r\n";
  $headers .= "Content-type:text/html;charset=UTF-8" . "\r\n";
  $headers .= 'From: <'.$senderEmailFinder[0].'>' . "\r\n";
if (mail("$creatorEmailFinder[0]","$subject","$emailContent", $headers)) {
    $response.='"stuvationConnectOperation":"done"';
}else{
    $response.='"stuvationConnectOperation":"fail"';
}
$response.="}";
}
