...
$lastDateOfJoining = date("Y-m-d", strtotime($request->lastDateOfJoining));
...
$prototypeAvailable =$request->prototypeAvailable;
$referenceLink =$request->referenceLink;
$expectedBudget =$request->expectedBudget;
$lookingProfessorGuidance =$request->lookingProfessorGuidance;
$lookingMentorship =$request->lookingMentorship;
$lookingStudentPartner =$request->lookingStudentPartner;
$teamSize =$request->teamSize;
$lookingSponsorship =$request->lookingSponsorship;
$noOfSponsors =$request->noOfSponsors;
$projectType =$request-> projectType;
$userId = $request->userDetails->userid;



  $subject="Stuvation Created. <Approve/Decline>";
...
$response.="{";
$authCode = rand();
$stuvationdetailsquery="insert into stuvationdetails (
projectType	,
projectTitle	,
abstract	,
projectStatus	,
...
referenceLink	,
expectedBudget	,
lookingProfessorGuidance	,
lookingMentorship	,
lookingStudentPartner	,
teamSize	,
lastDateOfJoining	,
lookingSponsorship	,
noOfSponsors	, userid, createdby, updatedby, authcode) 
values($projectType, '$title', '$abstract', '$projectStatus', $technology, $subtechnology, 
'$idea', '$ideaDescription', '$prototypeAvailable','$referenceLink','$expectedBudget',
'$lookingProfessorGuidance','$lookingMentorship','$lookingStudentPartner'
,$teamSize,'$lastDateOfJoining','$lookingSponsorship',$noOfSponsors,$userId, $userId, $userId,  $authCode)";
//echo $stuvationdetailsquery;
if(mysqli_query($conn,$stuvationdetailsquery)){
    $response.='"stuvationdetailsquery":"done"';
}else{
    $response.='"stuvationdetailsquery":"failed"';
}
$stuvationId = mysqli_insert_id($conn);
$userDetailQuery= mysqli_query($conn,"select t1.userid, t1.username, t1.emailaddress, t2.name, t1.usertype, t1.emailverified from logindetails t1, basicinformation t2 where t1.userid=$userId and t1.userid = t2.userid");
$userDetailQueryRes=mysqli_fetch_row($userDetailQuery);

$approveEmailUrl="http://edubee.accrosian.com/assets/services/updateStuvationStatus.php?stuvationId=".$stuvationId."&approvalStatus=approved&authCode=".$authCode;
$declineEmailUrl="http://edubee.accrosian.com/assets/services/updateStuvationStatus.php?stuvationId=".$stuvationId."&approvalStatus=declined&authCode=".$authCode;
$articleDetails='
<h5>Stuvation Details:</h5>
...
<br/>
<b>Subtmitted By</b> : '.$userDetailQueryRes[3].'
';
$emailContent = '<head>
